...
sheet_id = 'AIzaSyCES-143IPcDvI36XYZ0ioOMUWPcYpZPcpvaxBl7D0rEbPveM'
...
def load_data_task(**kwargs):
    image_folder = "D:\\ASI\\24915_DogBreedPrediction\\images\\Images"  # Ścieżka do folderu z obrazami
    annotation_folder = "D:\\ASI\\24915_DogBreedPrediction\\annotations\\Annotation"  # Ścieżka do folderu z metadanymi
    data = load_data_with_paths(image_folder, annotation_folder)
    return data.to_dict()  # Zwraca dane jako słownik

def split_data_task(data_dict, **kwargs):
...
def upload_test_data_to_sheets(**kwargs):
    ...
    api_key = "AIzaSyCES-641xzQQRHVHGeh1iMmft6tJsLhuYs"  # Wstaw swój klucz API Google
    sheet_id = "143IPcDvI36XYZ0ioOMUWPcYpZPcpvaxBl7D0rEbPveM"  # Wstaw ID arkusza Google (znajdziesz w URL)
    sheet_name = "Test Data"  # Nazwa arkusza w Google Sheets
...
split_data_operator = PythonOperator(
    ...
    provide_context=True,
    op_kwargs={
        "data_dict": "{{ task_instance.xcom_pull(task_ids='load_data') }}"
    },
    dag=dag,
)
